...
Write-Host "====================================="
Write-Host "Mingtily Backend Startup"
Write-Host "====================================="
...
Write-Host "====================================="
Write-Host "Starting Mingtily Backend"
Write-Host "====================================="
...
# Check if mingtily is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for mingtily
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\mingtily\mingtily.exe",
    "$env:ProgramFiles\mingtily\mingtily.exe",
    "${env:ProgramFiles(x86)}\mingtily\mingtily.exe",
    "$env:APPDATA\mingtily\mingtily.exe"
)
...
if ($frontendInstalled) {
    Write-Host "Mingtily frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the Mingtily frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching Mingtily frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "Mingtily frontend launched successfully."
        }
    }
} else {
    Write-Host "Mingtily frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the Mingtily frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if mingtily is now installed and launch it
