...
$AppProjectName = "mingtily-backend"
$AppContainerName = "mingtily-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "mingtily-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server mingtily-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server mingtily-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "mingtily-backend"
        "backend" = "mingtily-backend"
        "meeting" = "mingtily-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "mingtily-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, mingtily-backend"
        ...
        $logArgs += @("whisper-server", "mingtily-backend")
    }
