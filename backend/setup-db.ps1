...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.mingtily\meeting_minutes.db",
        "$env:USERPROFILE\Documents\mingtily\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/mingtily-backend/*/backend/meeting_minutes.db"
    }
