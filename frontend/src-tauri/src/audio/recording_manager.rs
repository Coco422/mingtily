...
impl RecordingManager {
    ...
    pub fn apply_speaker_updates(
        &self,
        updates: &[crate::speaker_diarization::SpeakerLabelUpdate],
    ) {
        self.recording_saver.apply_speaker_updates(updates);
    }

    pub fn finalize_speaker_labels(&self) -> Vec<crate::speaker_diarization::SpeakerLabelUpdate> {
        self.recording_saver.finalize_speaker_labels()
    }

    pub fn write_transcripts_now(&self) -> Result<()> {
        self.recording_saver.write_transcripts_now()
    }
