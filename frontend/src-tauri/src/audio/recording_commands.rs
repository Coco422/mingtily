...
pub async fn stop_recording<R: Runtime>(
    ...
    // Put the stopped manager back temporarily so the transcript listener can persist
    // late worker emissions while the transcription queue drains.
    ...
    if let Some(task_handle) = transcription_task {
        ...
        // The listener and recording manager must remain alive until the worker exits;
        // otherwise a detached worker could emit transcripts after persistence is gone.
    ...
    // Keep this listener alive until every worker has emitted its final transcript.
    {
        ...
        if let Some(listener_id) = TRANSCRIPT_LISTENER_ID.lock().unwrap().take() {
            ...
            info!("✅ Transcript-update listener removed after workers completed");
        }
    }
    ...
    let (meeting_folder, meeting_name) = if let Some(mut manager) = manager_for_cleanup {
        ...
        if !speaker_updates.is_empty() {
            let _ = app.emit(
                ...
                serde_json::json!({ "labels": speaker_updates }),
            );
        }
        if let Err(error) = manager.write_transcripts_now() {
            warn!(
                "Failed to write final speaker labels to transcripts.json: {}",
                error
            );
        }
