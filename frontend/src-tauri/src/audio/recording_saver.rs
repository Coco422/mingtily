use crate::speaker_diarization::SpeakerLabelUpdate;
use anyhow::Result;
...
impl RecordingSaver {
    ...
    pub fn apply_speaker_updates(&self, updates: &[SpeakerLabelUpdate]) {
        if let Ok(mut segments) = self.transcript_segments.lock() {
            for update in updates {
                if let Some(segment) = segments
                    .iter_mut()
                    .find(|segment| segment.sequence_id == update.sequence_id)
                {
                    segment.speaker = update.speaker.clone();
                    segment.speaker_is_provisional = false;
                }
            }
        }
    }

    pub fn finalize_speaker_labels(&self) -> Vec<SpeakerLabelUpdate> {
        if let Ok(mut segments) = self.transcript_segments.lock() {
            segments
                .iter_mut()
                .map(|segment| {
    ...
    pub fn write_transcripts_now(&self) -> Result<()> {
        if let Some(folder) = &self.meeting_folder {
