...
pub struct TranscriptUpdate {
    ...
    #[serde(default)]
    pub speaker_is_provisional: bool,
}
...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        let transcription_provider = match super::engine::get_or_init_transcription_engine(&app)
            .await
        {
        ...
        for worker_id in 0..NUM_WORKERS {
            let provider_clone = transcription_provider.clone();
            let app_clone = app.clone();
            let work_receiver_clone = work_receiver.clone();
            let chunks_completed_clone = chunks_completed.clone();
            let input_finished_clone = input_finished.clone();
            let chunks_queued_clone = chunks_queued.clone();
            let speaker_paths_clone = speaker_model_paths.clone();

            let worker_handle = tokio::spawn(async move {
                info!("👷 Worker {} started", worker_id);

                // PRE-VALIDATE model state to avoid repeated async calls per chunk
                let initial_model_loaded = provider_clone.is_model_loaded().await;
                let current_model = provider_clone
                    .get_current_model()
                    .await
                    .unwrap_or_else(|| "unknown".to_string());

                let engine_name = provider_clone.provider_name();
                ...
                let mut speaker_session = match speaker_paths_clone {
                    ...
                    .await
                    {
                        ...
                        Ok(Err(error)) => {
                            warn!("Unable to initialize realtime speaker labels: {}", error);
                            None
                        }
                        Err(error) => {
                            warn!("Realtime speaker initialization task failed: {}", error);
                            None
                        }
                    },
                    None => None,
                ...
                loop {
                    ...
                    match chunk {
                        Some(chunk) => {
                            ...
                            if !provider_clone.is_model_loaded().await {
                            ...
                            let labeled_chunks =
                                prepare_speaker_chunks(&mut speaker_session, chunk).await;
                            for labeled_chunk in labeled_chunks {
                                let chunk = labeled_chunk.chunk;
                                let speaker = labeled_chunk.speaker;
                                let chunk_timestamp = chunk.timestamp;
                                let chunk_duration =
                                    chunk.data.len() as f64 / chunk.sample_rate as f64;

                                // Transcribe with provider-agnostic approach
                                match transcribe_chunk_with_provider(provider_clone.as_ref(), chunk)
                                    .await
                                {
                                    Ok((transcript, confidence_opt, is_partial)) => {
                                        // Provider-aware confidence threshold
                                        let confidence_threshold =
                                            if confidence_opt.is_some() { 0.3 } else { 0.0 };
                                        ...
                                        let meets_threshold = confidence_opt
                                            .map_or(true, |c| c >= confidence_threshold);

                                        if !transcript.trim().is_empty() && meets_threshold {
                                            // PERFORMANCE: Only log transcription results, not every processing step
                                            info!("Worker {} accepted transcription: chars={} (confidence: {}, partial: {})",
                                            ...
                                            let update = TranscriptUpdate {
                                                ...
                                                speaker_is_provisional: speaker.is_some(),
                                            };
...
async fn prepare_speaker_chunks(
    session: &mut Option<RealtimeSpeakerSession>,
    chunk: AudioChunk,
) -> Vec<LabeledAudioChunk> {
    let Some(mut active_session) = session.take() else {
        return vec![LabeledAudioChunk {
            chunk,
            speaker: None,
        }];
    };
    ...
    let chunk_start = chunk.timestamp;
    let result = tokio::task::spawn_blocking(move || {
        let result = active_session.process_chunk(&samples, chunk_start);
        (active_session, result)
    })
    .await;

    match result {
        Ok((returned_session, Ok(segments))) => {
            *session = Some(returned_session);
            ...
            let device_type = chunk.device_type;
            segments
                .into_iter()
                .map(|segment| LabeledAudioChunk {
                    chunk: AudioChunk {
        ...
        Ok((returned_session, Err(error))) => {
            *session = Some(returned_session);
            warn!(
                "Realtime speaker labeling failed; using unlabeled chunk: {}",
                error
            );
            vec![LabeledAudioChunk {
                chunk,
        ...
        Err(error) => {
            warn!(
                "Realtime speaker task failed; disabling labels for this session: {}",
                error
            );
            vec![LabeledAudioChunk {
                chunk,
...
async fn transcribe_chunk_with_provider(
    ...
    // Speaker boundaries and VAD flushes can produce tiny fragments. They are
    // expected control-flow noise, not actionable transcription failures.
    if speech_samples.len() < MIN_TRANSCRIPTION_SAMPLES {
        info!(
            "Audio chunk {} has only {} samples; skipping transcription",
            chunk.chunk_id,
            speech_samples.len()
        );
        return Err(TranscriptionError::AudioTooShort {
            samples: speech_samples.len(),
    ...
    match provider.transcribe(speech_samples, language).await {
        Ok(result) => {
            ...
            let confidence_str = match result.confidence {
                Some(confidence) => format!("confidence: {confidence:.2}"),
        ...
        Err(error) => {
            match &error {
                TranscriptionError::AudioTooShort { .. } => info!(
                    "{} skipped short audio chunk {}: {}",
                    provider.provider_name(),
                    chunk.chunk_id,
                    error
                ),
...
mod tests {
    ...
    impl TranscriptionProvider for RecordingProvider {
        ...
        ) -> Result<super::super::TranscriptResult, TranscriptionError> {
            self.called.store(true, Ordering::SeqCst);
            Ok(super::super::TranscriptResult {
    ...
    #[tokio::test]
    async fn short_audio_is_skipped_before_provider_inference() {
        ...
        let error = transcribe_chunk_with_provider(&provider, chunk)
            .await
            .unwrap_err();
