...
impl<R: Runtime> SystemNotificationHandler<R> {
    ...
    pub async fn show_notification(&self, notification: Notification) -> Result<()> {
        ...
        match builder.show() {
            Ok(_) => {
                log_info!("Successfully showed Tauri system notification");
                Ok(())
            }
            Err(e) => {
