...
pub async fn show_notification(
    ...
    log_info!("Showing a custom notification");
...
pub async fn show_recording_started_notification<R: Runtime>(
    ...
    log_info!(
        "Attempting to show recording-started notification (custom title: {})",
    ...
    if let Some(manager) = manager_lock.as_ref() {
        ...
        match initialize_notification_manager(app_handle.clone()).await {
            ...
            Err(e) => {
                ...
                let title = "Mingtily";
                let zh_cn = crate::localization::is_zh_cn(app_handle);
                let body = match (meeting_name, zh_cn) {
                    ...
                    (None, false) => "Recording has started. Please inform others in the meeting that you are recording.".to_string(),
                };

                log_info!("Using direct Tauri recording-started notification fallback");
...
pub async fn show_recording_stopped_notification<R: Runtime>(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        ...
        let title = "Mingtily";
        let body = if crate::localization::is_zh_cn(app_handle) {
            "录音已停止"
        } else {
            "Recording has stopped"
        };

        log_info!("Using direct Tauri recording-stopped notification fallback");
