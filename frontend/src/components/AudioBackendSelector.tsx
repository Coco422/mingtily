...
export function AudioBackendSelector({
  ...
  useEffect(() => {
    const loadBackends = async () => {
      try {
        ...
        setError(t('recordings.backend.loadFailed'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBackendChange = async (backendId: string) => {
    try {
      ...
      setError(t('recordings.backend.changeFailed'));
    }
  };
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-gray-900 text-white rounded-lg shadow-lg">
              <p className="font-semibold mb-1">{t('recordings.backend.captureMethods')}</p>
              ...
              <p className="mt-2 text-gray-300">
                {t('recordings.backend.tryDifferent')}
              </p>
            </div>
          )}
        </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {currentBackend === backend.id && (
                    <span className="text-xs font-medium text-blue-600 bg-blue-100 px-2 py-0.5 rounded">
                      {t('recordings.backend.active')}
                    </span>
                  )}
                  {isCoreAudio && (
                    <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded">
                      {t('recordings.backend.disabled')}
                    </span>
                  )}
                </div>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• {t('recordings.backend.systemOnly')}</p>
        <p>• {t('recordings.backend.microphoneDefault')}</p>
        <p>• {t('recordings.backend.nextSession')}</p>
      </div>
    </div>
  );
}
