...
export function ChunkProgressDisplay({
  ...
  const { t } = useTranslation(['common', 'meeting', 'recording']);
  ...
  const formatDuration = (ms: number) => {
    ...
    if (hours > 0) {
      return t('meeting:durationHours', { hours, minutes: minutes % 60, seconds: seconds % 60 });
    } else if (minutes > 0) {
      return t('meeting:durationMinutes', { minutes, seconds: seconds % 60 });
    } else {
      return t('meeting:durationSeconds', { seconds });
    }
  };

  const formatTimeRemaining = (ms?: number) => {
    if (!ms || ms <= 0) return t('meeting:calculating');
    ...
    <div className={`bg-white border border-gray-200 rounded-lg p-4 ${className}`}>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-gray-700">
            {t('meeting:chunksCompleted', { completed: progress.completed_chunks, total: progress.total_chunks })}
          </span>
          <span className="text-sm font-medium text-gray-700">
      ...
      <div className="grid grid-cols-4 gap-4 mb-4 text-sm">
        <div className="text-center">
          ...
          <div className="text-gray-600">{t('meeting:completed')}</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-gray-600">{t('common:processing')}</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-gray-600">{t('meeting:pending')}</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-gray-600">{t('meeting:failed')}</div>
        </div>
      </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-blue-800">
              {t('meeting:estimatedTimeRemaining', { time: formatTimeRemaining(progress.estimated_remaining_ms) })}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        <h4 className="text-sm font-medium text-gray-700 mb-2">
          {t('meeting:recentChunks', { shown: Math.min(progress.chunks.length, 10), total: progress.total_chunks })}
        </h4>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium">
                      {t('meeting:chunk', { number: chunk.chunk_id })}
                    </span>
                    {chunk.duration_ms && (
                ...
                {chunk.error_message && (
                  <div className="mt-1 text-red-700 text-xs">
                    {t('meeting:errorWithMessage', { error: chunk.error_message })}
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-green-800">
              {t('meeting:processingCompleted', { total: progress.total_chunks })}
            </span>
          </div>
        </div>
      )}
  ...
  return (
    <div className={`bg-gray-50 border border-gray-200 rounded-lg p-3 ${className}`}>
      ...
      <div className="text-xs text-gray-600">
        {t('meeting:chunksProgress', { completed: progress.completed_chunks, total: progress.total_chunks })}
        {progress.processing_chunks > 0 && (
          <span className="ml-2 text-blue-600">
            ({t('meeting:chunksProcessing', { count: progress.processing_chunks })})
          </span>
        )}
      </div>
    </div>
