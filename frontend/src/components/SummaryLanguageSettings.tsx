...
export function SummaryLanguageSettings() {
  ...
  const languageLabel = (code: string) => labelForCode(code, i18n.resolvedLanguage || i18n.language);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-gray-900">{t('services.summary.language')}</h3>
      </div>
      <p className="text-sm text-gray-600 mb-4">
        {t('services.summary.languageDescription')}
      </p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {recents.map((code) => {
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={isPinned ? t('services.summary.unpinLanguage', { language: languageLabel(code) }) : t('services.summary.pinLanguage', { language: languageLabel(code) })}
                aria-pressed={isPinned}
                title={isPinned ? t('services.summary.unsetDefault') : t('services.summary.setDefault')}
              ...
              <button
                type="button"
                aria-label={t('services.summary.removeLanguage', { language: languageLabel(code) })}
        ...
        <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
          <PopoverTrigger asChild>
            ...
            >
              ＋ {t('services.summary.addLanguage')}
            </button>
          </PopoverTrigger>
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? t('services.summary.defaultLanguageHint', { language: languageLabel(pinned) })
          : t('services.summary.noDefaultLanguageHint')}
      </p>
    </div>
  );
}
