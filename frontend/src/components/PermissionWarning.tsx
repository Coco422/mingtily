        ...
        <Alert variant="destructive" className="border-amber-400 bg-amber-50">
          ...
          <AlertTitle className="text-amber-900 font-semibold">
            <div className="flex items-center gap-2">
              ...
              {!hasMicrophone && !hasSystemAudio ? t('permissionsRequired') : !hasMicrophone ? t('microphoneRequired') : t('systemAudioRequired')}
            </div>
          </AlertTitle>
          {/* Action Buttons */}
              ...
              <>
                ...
                <div className="space-y-2 text-sm mb-4">
                  ...
                  <ul className="list-disc list-inside ml-2 space-y-1">
                    <li>{t('micConnected')}</li>
                    <li>{t('micPermission')}</li>
                    <li>{t('micNotBusy')}</li>
                  </ul>
                </div>
              </>
            ...
            {!hasSystemAudio && (
              <>
                <p className="mb-3">
                  {hasMicrophone
                    ? t('systemUnavailable')
                    : t('systemAlsoUnavailable')}
                </p>
                {isMacOS && (
                  <div className="space-y-2 text-sm mb-4">
                  <p className="font-medium">{t('systemSetup')}</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                    <li>{t('installVirtualDevice')}</li>
                    <li>{t('grantScreenPermission')}</li>
                    <li>{t('configureAudio')}</li>
                    </ul>
                  </div>
                )}
              </>
