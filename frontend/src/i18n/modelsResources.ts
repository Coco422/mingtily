...
export const modelsResources = {
  'en-US': {
    ...
    offlineImport: { title: 'Install an offline model', description: 'Import a supported model downloaded on another device. Mingtily recognizes registered models, verifies every file, and copies them into private model storage.', file: 'Choose model file', archive: 'Choose archive', directory: 'Choose extracted folder', hint: 'Supported: registered Whisper GGML .bin files, Sherpa ONNX .zip or .tar.bz2 packages, and complete extracted Sherpa directories. Only exact registered artifacts pass verification.', success: '{{model}} imported and verified', failed: 'Could not import the offline model' },
  ...
  'zh-CN': {
    ...
    offlineImport: { title: '离线安装模型', description: '导入已在其他设备下载好的受支持模型。Mingtily 会识别已登记模型、逐文件校验，再复制到应用自己的模型目录。', file: '选择模型文件', archive: '选择压缩包', directory: '选择已解压目录', hint: '当前支持已登记的 Whisper GGML .bin 文件、Sherpa ONNX .zip 或 .tar.bz2 原始包，以及完整解压后的 Sherpa 目录；只有与登记规格完全一致的文件才能通过校验。', success: '{{model}} 已导入并通过校验', failed: '离线模型导入失败' },
