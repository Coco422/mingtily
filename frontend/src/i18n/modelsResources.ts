...
export const modelsResources = {
  'en-US': {
    ...
    sherpa: {
      ...
      offlineImport: { title: 'Install an offline model', description: 'Already downloaded the model on another device? Import the original supported archive or its extracted model directory. Mingtily recognizes the model automatically, verifies every file, and copies it into the app’s private model storage.', archive: 'Choose archive', directory: 'Choose extracted folder', hint: 'Supported archives: the pinned .zip or .tar.bz2 packages listed by Mingtily. For separate weight files, choose the folder that contains the complete model. Existing files are replaced only after verification succeeds.', success: '{{model}} imported and verified', failed: 'Could not import the offline model' },
  ...
  'zh-CN': {
    ...
    sherpa: {
      ...
      removed: '已删除 {{model}}',
      offlineImport: { title: '离线安装模型', description: '如果你已经在其他设备下载好模型，可直接导入受支持的原始压缩包，或导入解压后的完整模型目录。Mingtily 会自动识别模型、逐文件校验，再复制到应用自己的模型目录。', archive: '选择压缩包', directory: '选择已解压目录', hint: '压缩包支持 Mingtily 已登记并锁定版本的 .zip 或 .tar.bz2；如果手上是多个权重文件，请选择包含完整模型的目录。只有全部校验通过后才会替换现有文件。', success: '{{model}} 已导入并通过校验', failed: '离线模型导入失败' },
