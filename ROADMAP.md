...
0.8.0 将实时转写、最终转写、说话人、标点、术语增强和会后处理统一为可校验、可解释的 Pipeline，并把长会议的重处理移入资源受控的持久后台任务。

- 全新安装可选择快速、均衡或高质量 Pipeline，也可以跳过后在 Services 中配置；高级用户可使用自定义模式控制每条处理路径和资源预算。
- 预设会展示推荐模型及安装状态，缺少资产时引导用户主动下载；Models 继续只管理资产，不因下载完成静默切换运行配置。
- Rust 侧统一解析模型能力、Beta Gate、平台、语言、内置标点、说话人输出、输入模式、线程与内存预算，并返回实际 Pipeline、避让项和 fallback 原因。
- 模型自带标点时跳过外部标点；模型自带 speaker 时避免重复 diarization；continuous、whole-file、provisional 和 finalized 路径保持明确边界。
- 停止录音不再同步等待整场 speaker refinement。ASR 重算与说话人校正持久化为后台任务，支持依赖、恢复、暂停、继续、取消、重试和事务性结果替换。
- 长会议按有界窗口解码、重采样和推理，每个窗口保存 checkpoint 后释放 samples；任务在录音期间让路，并按 CPU、模型线程和内存预算限制并发。
- 会议详情展示跨路由处理进度和单次重新处理入口；实时 provisional 文本仍不进入数据库、术语替换或标点恢复。
- Beta 设置收敛到真正的实验模型和文件处理能力；Pipeline 作为核心产品能力始终可用。
...
- 有已验证 ModelScope 产物的语音识别、说话人分离、标点、Whisper 和内置摘要模型均优先使用 ModelScope，原有兼容来源保留为自动回退。
- 公共下载层统一支持 `.part` 断点续传、有界重试、停滞超时、严格大小和 SHA-256 校验，以及失败后不破坏已有安装的原子替换。
- Sherpa ONNX 模型可从本地压缩包或已解压目录导入；应用自动识别内置支持的精确版本并验证所有运行时文件。
- 已安装模型清单兼容多个受支持的上游版本，避免上游重新打包后误判正常模型，同时拒绝未经验证的内容进入原生推理层。

### 0.7.5：摘要 Prefill 等待体验

0.7.5 改善本地和外部模型生成会议摘要时的等待体验：首个 token 到达前会展示当前处理阶段和累计耗时，长会议的分段分析与合并也有明确反馈；这些状态由应用级任务中心保存，切换页面不会重置。

- 单次摘要会依次显示准备内容、模型读取会议、实时生成和可选翻译阶段，不用虚假百分比或预计完成时间掩盖 prefill 等待。
- 长会议会显示正在分析的分段和合并组数，中间摘要不会暴露到最终报告区域。
- 内置模型会说明首次输出前需要读取会议内容；外部 Provider 使用不暴露实现细节的通用等待提示。
- 任务开始时间以后台数据库记录为准，离开详情页后继续累计；首个可见 token 到达后自然切换到现有 Markdown 流式预览。

### 0.7.4：长会议本地摘要可靠性

0.7.4 修复长中文会议使用内置摘要模型时的上下文溢出：摘要处理器会使用所选 GGUF 模型的真实 tokenizer 计算完整提示词，自动进行安全分段和分层汇总；本地生成不再固定限制为 4096 输出 tokens，而是在模型上下文边界内生成到 EOS。

- 内置摘要的单次处理、分段处理、分层合并和最终报告都会按实际聊天模板后的 token 数检查容量，并为输出保留安全空间。
- `llama-helper` 新增本地 token 计数协议；即使上层预检失效，超长提示词也会在进入 llama.cpp batch 前返回明确错误。
- 本地摘要输出取消固定 4096-token 上限，仍由 32K 运行时上下文和请求超时提供硬性安全边界。

0.7.4 同时延续 0.7.3 的模型下载重试、macOS 录音降级、后台摘要、会议级说话人映射、自定义术语和本机开发隔离保护。

- Whisper 模型下载支持 `.part` 断点续传、终态校验与原子替换；失败、取消后会正确释放任务锁，可立即重试。
- macOS 系统音频权限与 CoreAudio 启动增加有界超时；系统音频不可用时明确提示并降级为麦克风录音，不再阻塞录音启动。
- 开发与测试环境复用构建期 FFmpeg sidecar，避免在无网络测试中重复下载或长时间等待。

- 本地开发与本地打包默认使用 `Mingtily Dev` / `com.mingcheng.mingtily.dev`，与正式应用的数据、日志、权限和 WebView 存储完全隔离；开发版禁用应用更新。
- Release 工作流显式恢复 `Mingtily` / `com.mingcheng.mingtily`，并在 CI 中校验本地和正式身份边界。
- 新增 0.6.2 数据库升级回归测试与已发布迁移 checksum 保护；旧程序遇到更新版本数据库时显示可操作提示并安全退出，不再 abort 或建议清空数据。
- Mingtily 浅蓝品牌、独立 bundle identifier 和全新本地数据空间。
...
目标：在不扩大产品边界的前提下，继续提高 0.7.4 的真实设备可靠性和社区可用性。

- 扩大后台摘要恢复、会议级说话人映射、自定义术语、录音、导入和重新转写在真实设备上的回归覆盖。
...
验收标准：主要录音路径无数据丢失；后台任务和会议级映射可从持久状态恢复；三平台构建状态明确；无签名安装包的限制有清晰说明；仓库不依赖 Meetily 的私有服务或凭证。macOS Developer ID、notarization 和 Gatekeeper 发布验证不作为 0.7.x 门禁。
...
- **说话人体验**：在现有会议级重命名、合并、拆分和 revision 冲突保护基础上，继续改善长会议统计、批量操作和可访问性；默认不做跨会议声纹记忆。
